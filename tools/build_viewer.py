...
def build_mobs():
    ...
    for path in sorted(glob.glob(os.path.join(ROOT, 'Assets', 'Resources', 'MobData', '*.asset'))):
        ...
        mobs[name] = {
            ...
            'elements': elements,
            'talk': talk,
            'codexQuote': quotes.get(name, ''),
        }
    return mobs


# 圖鑑台詞：跟筆記本（Notes.txt）同一套，是 Naninovel 的文字檔，只寫中文，
# 翻譯另外放 Localization/語言/Text/MonsterQuotes.txt。代號＝MobData 檔名。
# 檢視器存檔時由 serve.py 寫回同一份（見 save_mob）。
QUOTES_FILE = os.path.join(ROOT, 'Assets', 'Naninovel', 'Resources', 'Naninovel', 'Text', 'MonsterQuotes.txt')


def read_monster_quotes():
    quotes = {}
    if not os.path.isfile(QUOTES_FILE):
        return quotes
    for line in io.open(QUOTES_FILE, encoding='utf-8-sig').read().splitlines():
        if line.startswith(';') or ': ' not in line:
            continue
        key, value = line.split(': ', 1)
        quotes[key.strip()] = value
    return quotes
