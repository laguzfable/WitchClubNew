...
public class CSVReader : MonoBehaviour 
{
	...
	static public string[,] SplitCsvGrid(string csvText)
	{
		...
		int dataLength = rawLines[0].Count(c => c == ',');
		string cacheStr = string.Empty;
		for (int i = 1; i < rawLines.Length; i++)
        {
			cacheStr += rawLines[i];

			if(string.IsNullOrEmpty(rawLines[i]) || cacheStr.Count(c => c == ',') < dataLength || cacheStr.Count(c => c == '"') % 2 > 0)
			{
				continue;
				//cacheStr += rawLines[i];
			}
			...
			{
				collectionList.Add(cacheStr);
				cacheStr = string.Empty;
			}
		}
		string[] lines = collectionList.ToArray(); //csvText.Split("\n"[0]);
